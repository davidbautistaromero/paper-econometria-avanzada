...
class SecopProponentesExtractor:
    """Extractor for SECOP Proponentes data from Socrata API"""
    
    def __init__(self, base_url="https://www.datos.gov.co/resource/hgi6-6wh3.json"):
    ...
    def extract_all_data(self, output_file="datasets/01_raw/secop_proponentes.csv", 
                        save_chunks=True):
        """
        ...
        while True:
            try:
                ...
                if save_chunks and len(all_data) >= 100000:
                    chunk_file = output_path.parent / f"chunk_proponentes_{chunk_number:04d}.csv"
    ...
    def merge_chunks(self, chunks_dir, output_file):
        ...
        chunk_files = sorted(chunks_dir.glob("chunk_proponentes_*.csv"))
...
def main():
    ...
    extractor = SecopProponentesExtractor()
    
    # Extract all data
    extractor.extract_all_data(
        output_file="datasets/01_raw/secop_proponentes.csv",
        save_chunks=True  # Save intermediate chunks for safety
    )
    
    # Print summary
    output_file = Path("datasets/01_raw/secop_proponentes.csv")
